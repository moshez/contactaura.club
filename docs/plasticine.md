...
- [Fantasy Adaptation](#fantasy-adaptation)
  - [Weapon Proficiencies](#weapon-proficiencies)
  - [Magic Spells](#magic-spells)
  - [Thief Abilities](#thief-abilities)
  - [Fantasy Species](#fantasy-species)
  - [Campaign Guidelines](#campaign-guidelines-1)
- [Best Practices](#best-practices)
  - [What Is a Tabletop Role-Playing Game?](#what-is-a-tabletop-role-playing-game)
...
Plasticine uses one core mechanic.
How "good" something is is measured in pips.
Pips define how to roll for things:

* Divide pips in three, with remainder.
* The result is the number of dice.
* The remainder is the bonus.
...
High rolls are always better.
Some rolls are pass/fail:
in that case,
the difficulty is represented as a threshold.
For something with 10 pips,
a difficulty of 15 is significant,
but achievable:
you need to roll 3d6+1 and get 15 or more.

Some rolls are vs. rolls:
higher rolls wins.
Something with 10 pips against something with 8 pips
means

* One side: roll 3d6+1
* Other side: roll 2d6+2
* The first side has better chances of winning.

Some rolls are "effect" rolls:
in those cases,
the roll's value translates
...
- An initial amount (the GM will determine how many)
- Problems (the GM determines minimum/maximum amounts)
- Adventuring

On the character sheet, denote "Total CP" and "Unspent CP" separately.

Everything is bought in pips.
For simplicity,
on the character sheet,
we pre-calculate it in dice.
This makes it faster to run action scenes
during game play.
...
Most attributes cost 1 pip/CP. Action costs 3 CP/pip.

The following attributes exist:

- Action (ACT) - 3 CP/pip
- Accuracy (ACC) - 1 pip/CP
- Defense (DEF) - 1 pip/CP
- Perception (PER) - 1 pip/CP

It is recommended to buy each to at least 3d6.
...
Skills can be more or less specific.
Specificity can give bonuses or penalties.
Before you compute the bonus or penalty,
define the "natural specificity".
Natural specificity is determined by the task itself, not the character's skill


* For every step "up" from the natural specificity, there's a -1 penalty
* For every step "down" from the natural specificity, there's
  * +1 for compatible specificity
  * -1 for incompatible specificity
 
Example: A character buys: "Systems: Modern Linux computers" to 3d6.
They encounter a modern windows computer.
The GM decides the natural specificity is "computer".
Because of that:

* Modern (+1)
* Windows/Linux incompatibility (-1)

Net bonus/penalty: 0

If the character bought: "Systems: Electronics"
that's one step up in specificity,
so the character would be at -1 penalty.

Example: Character has: "Background: Science: Physics" at 3d6

Explaining quantum mechanics:
- Natural specificity: "Science"
- Matching specificity: "Science: Physics"
- Modifier: +1

Identifying a chemical compound:
- Natural specificity: "Chemistry"
- Chemistry is incompatible with Physics
- Modifier: -1

Calculating orbital trajectories
- Natural specificity: "Physics"
- Modifier: +0

Answering general science trivia
- Natural specificity: "Science"
- One step up from "Science: Physics"
- Modifier: -1
...
This applies to systems "found in the field". Any system that is in regular use should be bought as a helper or power.
...
Evasion covers any attempt to throw off someone.
...
As far as powers are concerned, Body costs 1 pt/1 CP. This means that it is drained, improved, or resisted at though it cost 1 pt/1 CP. Similarly, ACT costs 3 CP/pip for adjustment purposes, reflecting its higher value.
...
- **Improve (double cost):** Increase attribute instead.
- **Ranged:**
  - 30m: double cost
  - more than 30m: multiple cost by 2.5
  - Ignore obstacles: double cost.
- **AoE:** Multiply cost by 1+log_2(1 + radius)
  - Selective AoE: Double that cost.
- **Decreased healing:** Heals more slowly, 1 + log_2(slowness factor)
- **Use different resistance:** double cost
  - This means adjusting a statistic, but using another statistic's resistance.
  - Each "difference" will cause another doubling: Body and attributes are all considered the same. However, using resistance to Movement power adjustment to target Body adjustment is another doubling. Using "resistance to resistance to Movement power adjustment" to target Body adjustment results in two doublings (for a total of 8x the original cost).
- **Constant:** double cost (after activating, keeps adjusting until turned off)
...
- **Ignore intervening:** double cost
...
- **Ranged:** Multiply cost by 1+log_2(range in meters)
...
- **Range:** Multiply cost by 1+log₂(range in meters)
- **Penetrating:** Double cost to detect through barriers or obstacles
- **Analyze:** Double cost to automatically understand/analyze what is detected
- **Constant:** Double cost to maintain detection without requiring active focus
- **AoE:** Multiply cost by 1+log₂(radius in meters) to detect in an area

#### Signal

**Base cost: 1 CP per intensity level**
- 3 pips become a D, as with other powers
...
- **Range:** Multiply cost by distance factor
- **Multiple recipients:** Multiply cost by 1+log₂(number of targets)
- **Penetrating:** Double cost to signal through barriers
- **Complex signals:** Double cost for detailed information transfer
- **Constant:** Double cost to maintain signal without requiring active focus

#### Systems

All powers can be bought with the following "systems":

- **Flexible advantage:** Can use any combination of advantages, up to half cost.
- **Variant:** Pay 10% of the power cost, rounded down with a minimum of 1 CP, for another variant of the same power, as long as total cost is the same as lower.
...
- 0.25 for "barely limiting"
- 0.5 for "severely limiting"
- 1 for "extremely limiting"

Limitations can include:

- Require roll
- Limited uses
- Only functions in specific circumstances
- Requires item
- Increased healing
- Extra time
- Concentration
...
**Requires common item**
- "Requires gun" when you're a gunfighter who always carries one
- Maybe 1 in 8 fights you're caught without it (disarmed, ambushed while unarmed)
- 12.5% complete prevention ≈ 25% reduction in value

**Gestures**
- Need hands free to use power
- Rarely bound/grappled in typical fights
- When it matters, costs actions to free hands or position (≈ 25% efficiency loss)

**Verbal component**
- Need to speak/chant
- Occasionally gagged, underwater, or need stealth
- ≈ 15% of situations where you can't use it or it creates complications

**Only against common target type**
- "Only against living creatures" when 90% of your enemies are people
- The 10% where you face constructs/undead, power is useless
- But that's only slightly limiting since it's rare

**Extra time (minor)**
- "Requires 1 extra action to aim/prepare"
- In 5-action fights, this reduces your effective uses from 5 to 3 (40% reduction)
- But many fights last longer, averaging closer to 25% reduction

##### 0.5 Limitation (33% less useful)

**Requires specific item**
- "Requires THIS sword" (not any sword)
- If lost/stolen, major problem until recovered
- Happens maybe once every 3-4 sessions (≈ 25-33% downtime)

**Requires skill roll**
- Add uncertainty to every use
- With reasonable skill, ≈ 20-30% failure rate
- Plus the psychological cost of unreliability

**Only on ground/flat surface**
- Fighting in trees, on slopes, while flying = can't use
- Maybe 25-30% of encounters have problematic terrain

**Limited uses (moderate)**
- "3 uses per session" when you'd ideally use it 4-5 times
- Forces conservation, sometimes can't use when optimal
- Effective 30-40% reduction in utility

**Extra time (significant)**
- "Requires 2 actions to prepare/activate"
- In typical 6-action fight, reduces uses from 6 to 2 (67% reduction)
- But can sometimes prepare before combat, averaging to ≈ 35% reduction

**Only against moderately common type**
- "Only against living creatures" when you face undead/constructs 25-30% of time
- Clear 25-30% complete prevention

##### 1.0 Limitation (50% less useful)

**Only in uncommon circumstance**
- "Only in darkness" when half your fights are in daylight
- "Only underwater" in a campaign with occasional water missions
- Direct 50% prevention rate

**Cool down (exhausts for 3 actions)**
- In 6-action fight: use once, wait 3, use again = 2 uses instead of 6
- 67% reduction in rapid combats, averages to ≈ 50% across campaign

**Only against narrow target type**
- "Only against demons" in a campaign where demons are 40-50% of enemies
- Half the time it's completely useless

**Requires rare/targeted item**
- "Requires artifact that enemies know to target and steal"
- Frequently lost, stolen, or unavailable
- ≈ 50% uptime across campaign

**Limited uses (severe)**
- "Once per session" when you'd want it 2-3 times per session
- Forces you to save it, often can't use optimally
- Effective 50% reduction in value
...
- Reduce PER, 3d6, Area of Effect: 4m: 15 * (1+1): Raw strength 30.
- Limitations: Only against sight (1), Only to avoid perceiving the character (1): (1+1+1)=3
- Cost: 10 CP
...
- 2d6 Defense (6)
  - Limitations: Requires using action (1), Only against Hand-to-Hand (0.5)
- 3d6 Increase ACT (30)
  - Limitations: Self-only (0.5), Only if an attack missed (1)
- Total: 16
...
- Move: 30m
  - Raw strength: 6 CP
- Limitations: Speed inertia: can increase/reduce speed 5m/round (0.5), Direction inertia: can do one turn unit per round (0.5)
- Total cost: 6/(1+0.5+0.5) = 3
...
- 4d6 System (12)
  ...
  - 1 Defense
  - 1 Act
- Total raw strength: 60
- Limitation: Requires skill roll (0.5 Limitation)
- Cost: 60/1.5=40
...
- Resistance: Body, 1CP (5 Raw strength)
- Limitation: Only against damage from breathing water (2)
- Total cost: 5/(1+2) = 2
...
- Detect: Thoughts, 2d6 (6 CP raw strength)
  - Limitations: Requires concentration (0.5), Line of sight only (0.5)
  - Cost: 6/(1+0.5+0.5) = 3 CP
- Signal: Mental communication, 2d6 (6 CP raw strength)
  - Limitations: Requires concentration (0.5), Line of sight only (0.5)
  - Cost: 6/(1+0.5+0.5) = 3 CP
- Total: 6 CP
...
- Detect: Visual information, 3d6 (9 CP raw strength)
  - Limitations: Only in low light conditions (1), Only visual spectrum (0.5)
  - Cost: 9/(1+1+0.5) = 3.6 CP (rounded to 4 CP)
...
- Detect: Radio signals, 3d6 (9 CP raw strength)
  - Range: 1 km (x11 multiplier): 99 CP raw strength
  - Limitations: Only radio frequency (1), Requires equipment (0.5)
  - Cost: 99/(1+1+0.5) = 39.6 CP (rounded to 40 CP)
- Signal: Radio transmission, 3d6 (9 CP raw strength)
  - Range: 1 km (x11 multiplier): 99 CP raw strength
  - Limitations: Only radio frequency (1), Requires equipment (0.5)
  - Cost: 99/(1+1+0.5) = 39.6 CP (rounded to 40 CP)
- Total: 80 CP
...
- Detect: Imminent danger, 3d6 (9 CP raw strength)
  - Limitations: Only detects physical threats (0.5), Only works within 10m (0.5)
  - Cost: 9/(1+0.5+0.5) = 4.5 CP (rounded to 5 CP)
...
- Detect: Emotions, 2d6 (6 CP raw strength)
  - Limitations: Surface emotions only (0.5), Requires eye contact (0.5)
  - Cost: 6/(1+0.5+0.5) = 3 CP
...
- What powers, attributes, skills, or problems are considered the "free baseline"?
- Base CP
- Minimum/maximum CP from problems
- What is the maximum for attributes?
- What is the maximum raw strength for a power?
- What is the maximum level for skills? Is it different for background skills?
- What power systems, limitations, or advantages, are allowed?

Rough guidance:

- 25 CP is "normal human"
- 50 CP is "protagonist in a 'realistic' action movie or TV show"
- 100 CP is "superhero"
- 200 CP is "cosmic scale superhero"
...
In general,
characters should get
1-5CP per successful session.
This feels meaningful but not world-changing
in most genres
("realistic" to "cosmic").
...
- Hand to hand attack: Reduce Body by 1
- Running: Move 1m, only on ground

**Skills:**

- 3d6 Background: Professional
- +1 Native language
...
day-to-day
actions,
you don't need a roll.
Characters can open a door,
buy a snack,
or talk to a friend.
Plasticine comes into play
for
...
One of the dice is pre-designated as a wild die. As long as the wild die comes up six, it is rerolled. The total sum is added. If it comes up 1, the next roll is subtracted from the total and the 1 is discarded. If the next roll comes up 1, the entire roll is a "0".

### Rounds

A "round" is 1 second. Characters can only act in a round if they succeed in rolling ACT vs. Moderate (13).
They act in the order of the ACT roll.
...
If a character moves and then attacks before the opponent can act, the opponent is assumed to be facing the same direction. This means that if a character moves behind an opponent (moving 1m around them) and attacks, they are attacking from behind and gain a +1d6 bonus to Accuracy for that attack. This bonus represents the tactical advantage of an unexpected attack angle.

### Using Skills

Most skills are
"out of action sequences"
and can be considered free actions.
Resolution for skills has been described in the character section.
...
- **Reduce Body:** 3d6 (45 CP raw strength)
  - **Limitation:** Requires weapon (sword) (0.5)
  - **Cost:** 45/(1+0.5) = 30 CP
...
- **3d6 Reduce Body** (45 CP raw strength)
- **4d6 System** (12 CP)
  - Can be distributed between Accuracy, Defense, and Act when using the sword
- **Total raw strength:** 57 CP
- **Limitation:** Requires weapon (sword) (0.5)
- **Cost:** 57/1.5 = 38 CP
...
- **Aggressive Style:** 3d6 Accuracy, 1d6 Act
- **Defensive Style:** 3d6 Defense, 1d6 Accuracy
- **Balanced Style:** 2d6 Accuracy, 2d6 Defense
- **Swift Style:** 2d6 Act, 1d6 Accuracy, 1d6 Defense
...
- **Sniper Style:** 3d6 Accuracy, 1d6 Act (focus on hitting difficult targets)
- **Quick Shot:** 2d6 Act, 2d6 Accuracy (focus on firing rapidly)
- **Combat Archer:** 2d6 Accuracy, 2d6 Defense (for archers who might be in direct combat)

**Additional Optional Limitations:**

- Requires nocking an arrow (0.25)
- Limited ammunition (0.5)
- Reduced effectiveness in confined spaces (0.25)
...
- **Cool Down (1.0):** Exhausts magic of this type for the next three actions, preventing further spellcasting.
...
- **Improve Body:** 2d6 (30 CP raw strength)
  - Note: Using the "Improve" modifier doubles the base cost
  - **Advantages:** None (touch range is the default)
  - **Limitations:**
    - Requires gestures (0.5)
    - Requires verbalization/incantation (0.5)
    - Requires concentration (0.5)
    - Requires skill roll (0.5)
    - Cool down: exhausts healing magic for next three actions (1.0)
  - **Cost:** 30/(1+0.5+0.5+0.5+0.5+1.0) = 30/4 = 7.5 CP (round to 8 CP)

#### Utility Magic

##### Detect Magic

- **Detect:** Magical energy/auras, 3d6 (9 CP raw strength)
  - **Advantages:**
    - Range: 10m (×4 multiplier) = 36 CP
    - Analyze: To understand the type/purpose of magic (×2 multiplier) = 72 CP
  - **Limitations:**
    - Requires concentration (0.5)
    - Requires gestures (0.5)
    - Duration limited (only active for one action) (0.5)
  - **Cost:** 72/(1+0.5+0.5+0.5) = 72/2.5 = 28.8 CP (round to 29 CP)

##### Basic Detect Magic

- **Detect:** Magical energy/auras, 2d6 (6 CP raw strength)
  - **Advantages:**
    - Range: 5m (×3 multiplier) = 18 CP
  - **Limitations:**
    - Requires concentration (0.5)
    - Requires gestures (0.5)
    - Only detects presence/absence of magic, not details (0.5)
  - **Cost:** 18/(1+0.5+0.5+0.5) = 18/2.5 = 7.2 CP (round to 7 CP)

#### Support Magic

##### Inspire (Leadership Aura)

- **Improve Accuracy and Defense:** 2d6 each (60 CP raw strength)
  - **Advantages:**
    - Area of Effect: 3m radius (×2 multiplier) = 120 CP
  - **Limitations:**
    - Requires verbalization (speech, singing, etc.) (0.5)
    - Requires continuous performance/leadership (concentration) (0.5)
    - Cannot affect self (0.5) - optional
    - Cool down: cannot inspire again for three actions after ending (0.5)
  - **Cost:** 120/(1+0.5+0.5+0.5+0.5) = 120/3 = 40 CP

##### Battle Hymn (Offensive Inspiration)

- **Improve Accuracy only:** 3d6 (45 CP raw strength)
  - With same advantages and limitations as above
  - **Cost:** 90/3 = 30 CP

##### Protection Chant (Defensive Inspiration)

- **Improve Defense only:** 3d6 (45 CP raw strength)
  - With same advantages and limitations as above
  - **Cost:** 90/3 = 30 CP
...
- **Security/Hacking:** 3d6 (9 CP)
  - Specificity: Lock picking (mechanical locks)
...
- If trying to pick a medieval lock with this skill, there's no penalty
- If trying to pick a modern electronic lock, there would be incompatible specificity penalties
- If the character had taken a more general "Security/Hacking" without the lock picking specificity, they'd take a -1 penalty
...
- This represents the mild social challenges of being a non-human in many settings
- Could be upgraded to common (10 CP) in areas where that species faces frequent prejudice
- Could be upgraded to severe (10 CP) in areas with historical conflicts
...
- Defensive abilities with the limitation "Only underground" (0.5)
- Resistance to Body reduction with the limitation "Only against poisons" (0.5)

**Elves:**

- Perception abilities with the limitation "Only in natural settings" (0.5)
- Movement abilities with the limitation "Only through forests" (0.5)

**Halflings:**

- Evasion abilities with the limitation "Only when outnumbered" (0.5)
- Improved ACT with the limitation "Only when unnoticed" (0.5)
...
- For low-fantasy "gritty" campaigns: 30-50 CP
- For heroic fantasy campaigns: 50-80 CP
- For high-fantasy/epic campaigns: 80-120 CP

#### Base Package

Consider providing a standard base package for all characters that includes:

- 2d6 in all four attributes
- Basic communication skills
- Basic movement abilities

#### Power Scale

- Determine the maximum raw strength allowed for powers (recommended: 80-100 CP for heroic fantasy)
- Set limitations on how many dice can be allocated to a single attribute/skill/power

#### Magic Integration

- Decide how common magic is in your setting
- Determine whether all characters have access to magic or if it's restricted to certain character concepts
- Consider creating "schools" of magic using the system advantage
...
- Lines and veils (content boundaries)
...
- It would be entertaining for everyone
- It doesn't break the game's internal logic
- It doesn't overshadow other players' abilities
...
- A failed lockpicking attempt doesn't mean "the door stays locked"
- It means "you unlock it, but trigger a silent alarm"
- Or "you break your tools, and now guards are approaching"
...
- Start sessions with action or intrigue
- End sessions on unresolved tension when possible
- Alternate between combat, exploration, and social interaction
- Watch for player engagement and shift gears when attention wanes
...
- Create motivations for NPCs, not scripts
- Develop flexible encounters that can occur anywhere
- Build a toolbox of names, locations, and concepts to draw from
...
- If finding the rule takes less than 30 seconds, look it up
- If it would take longer, make a reasonable ruling and check later
- Document your rulings for consistency

##### House Rules

When modifying rules:

- Write down all house rules and share them with players
- Be consistent in their application
- Be willing to revise them if they aren't working
- Avoid changing rules mid-session unless absolutely necessary
...
- Directly address quiet players: "What does [character] think about this?"
- Create scenarios that highlight different character skills
- Notice when a player hasn't spoken in a while and bring them in
- Design challenges that require multiple approaches to solve
...
- Why does my character stay with this group?
- What do they contribute to the team?
- How do they form bonds with other characters?
...
- Take risks and make interesting choices
...
- Avoid monopolizing game time
- Support other characters' moments
- Find ways to involve quieter players
- React to others' actions and decisions
...
- Address it privately with the GM
...
- Communicating openly about preferences and boundaries
- Sharing responsibility for the group's enjoyment
- Being reliable about attendance and preparation

#### Safety Tools

Every table should have mechanisms for addressing uncomfortable content. These might include:

- **The X-Card:** A card that anyone can tap to signal discomfort
- **Lines and Veils:** Pre-established boundaries about content
- **Script Change:** Tools for pausing, rewinding, or fast-forwarding through scenes
- **Regular check-ins:** Brief discussions about how everyone is feeling about the game
...
- Players often enjoy well-crafted stories with clear direction
- Having some rails helps new players learn the game
- A completely open world can feel directionless and arbitrary
- Even the most acclaimed video games often have linear main quests
...
- The underpowered character who saves the day through creativity
- The temporarily overpowered ability that creates a spectacular scene
- The unexpected combination that changes the game state dramatically
...
- Fudging dice rolls for dramatic effect
- Ignoring certain mechanics for narrative payoff
- Creating special exceptions to rules for unique situations

As a player, ask:

- Could this moment be more interesting than strictly accurate?
- Would bending a rule create a better story?
- Am I putting fun and creativity above technical adherence?
...
- NCIS (contemporary crime investigation)
- Stargate (science fiction exploration)
- The Hunger Games (dystopian survival)
- John Wick (action thriller)
- Dark Elf Saga (high fantasy)

It doesn't work for:

- Stranger in a Strange Land (philosophical sci-fi where pivotal moments are ideological)
- Interview with a Vampire (gothic horror where social intrigue and existential dread drive the story)
- Sex and the City (social drama where relationships and status determine outcomes)
- Misery (psychological horror resolved through mental fortitude, not combat capability)
...
- **No trap options:** Cost reflects value aggressively
- **Background is free:** Narrative choices don't compete with mechanical effectiveness
- **Abstract powers:** Special effects serve story without creating balance concerns
- **Minimal stats:** Fast adjudication maintains narrative momentum
- **No class restrictions:** Build any concept that serves your story
...
Plasticine was designed to let you reverse-engineer action/adventure protagonists into game statistics.
Traditional systems force you to invent numbers for things the author never cared about. Plasticine lets you model exactly what you see in action sequences.
...
This means:
- 5 CP of Background skills ≈ 5 CP of ACC ≈ 5 CP of powers ≈ 5 CP from a Problem
- They serve different story purposes but contribute equivalent value to collaborative storytelling
- No "must-have" purchases that tax every build
- No "trap" options that waste points
- Character sheets reflect genuine concept priorities
...
- **ACT:** How often you act in action sequences
- **ACC:** How likely you are to successfully target something
- **DEF:** How likely you are to avoid being targeted
- **PER:** How likely you are to notice important things

Why these four? Because these are what action/adventure stories actually adjudicate:

- Can you act before the other person?
- Can you hit your target?
- Can you avoid being hit?
- Do you notice the clue/trap/ambush?

Everything else is either:
- **Background** (being smart, charismatic, strong enough for daily life)
- **Skills** (specific competencies)
- **Powers** (exceptional capabilities)
...
In Plasticine:
- Carrying reasonable equipment? Background (free)
- Carrying enormous loads? Buy Push power
- Breaking down doors? Reduce the door's structural integrity
- Wrestling someone? Reduce their Move power
...
**HERO has separate powers for:**
- Energy Blast (reduce STUN/BODY using physical defense)
- EGO Blast (reduce STUN/BODY using ego defense)
- Drain (temporarily reduce characteristic)
- Transfer (reduce one thing, increase another)

**But these are all variations of "reduce attribute" with different targeting.**

**Plasticine has one power: Adjust Attribute**
- Reduce or improve any attribute/skill/power
- Use modifiers for range, area, resistance type
- Special effects explain the narrative ("lightning bolt" vs "punch")

The same principle applies throughout:

- **Movement:** One power, limitations define how (flight, running, swimming, teleportation)
- **Detection:** One power, modifiers define what and how far
- **All "hiding" effects:** Reduce PER with different limitations
  - Invisibility: Reduce PER, only against sight
  - Disguise: Reduce PER, only against recognition
  - Stealth: Reduce PER, only against detection
  - Camouflage: Reduce PER, only to avoid notice
...
**D&D Style:**
- Buy DEX (expensive)
- Get bonus to Stealth, Acrobatics, Sleight of Hand, Initiative
- Creates forced correlation: dexterous people are stealthy AND acrobatic AND quick

**Reality in stories:**
- A cat burglar might be stealthy but not acrobatic
- A gymnast might be acrobatic but not stealthy
- A gunslinger might be quick but neither stealthy nor acrobatic
...
HERO has pages of specific limitations with precise values:
- "Costs Endurance (-1/4)"
- "Extra Time: 1 Turn (-3/4)"
- "Requires Skill Roll (-1/4 to -1/2)"
- Dozens more...
...
**Plasticine uses three buckets:**
- **0.25:** Barely limiting (minor inconvenience)
- **0.5:** Severely limiting (significant constraint on usage)
- **1.0:** Extremely limiting (rarely available)
...
**System Mastery in Traditional Games:**
- Finding undercosted feat combinations
- Exploiting specific rule interactions
- "Coffee-locks," "hexblade dips," slot economy optimization
- Trap options to avoid vs. must-haves to take

**Plasticine's Approach:**
- Cost ≈ story usefulness
- Simple enough that value is transparent
- Hard to exploit because there's no hidden complexity
- Judgment-based limitations prevent "finding the loophole"
...
**Sex and the City pivotal moments:**
- Does your conversation charm or alienate?
- Does your outfit make the right impression?
- Can you navigate the social hierarchy?
- Do you say the right thing at the critical moment?
...
If you need to fire a gun in horror, you've already lost. The pivotal moments are:
- Research and investigation (primary "powers")
- Sanity checks (primary resource being depleted)
- Understanding what you face
- Maintaining mental coherence

Even in psychological horror like *Misery*, the resolution isn't ACC vs DEF. It's "can Paul maintain enough mental fortitude and deception to survive?"
The final physical confrontation is narrative consequence of psychological endurance, not combat adjudication.
...
During action sequences, immortal and mortal characters function identically:
- Both roll DEF when targeted
- Both take Body damage when hit
- Both fall unconscious at Body 0
...
Action/adventure verisimilitude requires:
- Protagonists can survive multiple encounters
- But face real danger from competent opposition
- Individual fights have stakes
...
Because these are what action/adventure stories actually adjudicate. More attributes either:
- Create forced correlations that don't match story patterns
- Model things that should be skills/powers/background
- Add complexity without increasing story contribution
...
A character with Background: Medieval History might:
- Recognize the historical significance of a location
- Know which noble houses have feuds
- Understand castle architecture for tactical advantage
- Provide narrative hooks and roleplay moments
...
- **Daily-use gun:** Reduce Body power with "ranged" advantage and "requires gun" limitation
- **Signature sword:** Part of Reduce Body power with "requires specific weapon" limitation
- **Regular clothes, wallet, phone:** Background (free)
- **Vehicle used in missions:** Helper (vehicle)
- **Safe house for the team:** Helper (base)
...
This enables:
- Any power level to include both magic and tech
- Genre-mixing without balance concerns
- Player creativity without optimization penalties
...
- Model comprehensive reality
- Handle every possible genre
- Simulate social dynamics
- Adjudicate political intrigue
...
- Limitation values require calibration
- Situational modifiers need assessment
- Story impact determines pricing
...
- Fixed Body makes sense (maintains genre-appropriate danger)
- Four attributes make sense (model what stories actually adjudicate)
- Abstract powers make sense (mechanical effect separate from narrative description)
- Free background makes sense (narrative choices don't compete with effectiveness)
- Simple limitations make sense (support judgment, avoid false precision)
- Cost equals value makes sense (eliminate traps, align optimization with concept)
